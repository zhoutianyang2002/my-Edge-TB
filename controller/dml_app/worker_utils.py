...
def log (message: str,rec_time: bool = False,time_text = 'Time'):
	"""
	record some messages.
	"""
	if(rec_time):
		message = message + ', ' + time_text + '=' + str(record_time())
...
def record_time():
	global sum_time,pre_time
	cur_time = time.time()
	sum_time = sum_time + (cur_time - pre_time)
	pre_time = cur_time
	return sum_time