...
if __name__ == '__main__':
	testbed = default_testbed (ip='106.53.110.245', dir_name=dirName, manager_class=GlManager)
	...
	emu1 = testbed.add_emulator (name='emulator-1', ip='159.75.112.104', cpu=16, ram=64, unit='G')
	...
	en = testbed.add_emulated_node (name='n1', working_dir='/home/worker/dml_app',
		cmd=['python3', 'gl_peer.py'], image='dml:v1.0', cpu=1, ram=1, unit='G', emulator=emu1)
	...
	emu2 = testbed.add_emulator ('emulator-2', '111.230.154.108', cpu=128, ram=256, unit='G')
	for i in range (2, 3):
		en = testbed.add_emulated_node ('n' + str (i), '/home/worker/dml_app',
			['python3', 'gl_peer.py'], 'dml:v1.0', cpu=1, ram=1, unit='G', emulator=emu2)
	...
	emu3 = testbed.add_emulator ('emulator-3', '114.132.177.213', cpu=128, ram=256, unit='G')
	for j in range (3, 4):
		en = testbed.add_emulated_node ('n' + str (j), '/home/worker/dml_app',
			['python3', 'gl_peer.py'], 'dml:v1.0', cpu=1, ram=1, unit='G', emulator=emu3)
		# ${host_path} means /path/in/emulator-2/to/worker/dml_file in this example.
		en.mount_local_path ('./dml_file', '/home/worker/dml_file')
		en.mount_local_path ('./local_dataset/FASHION_MNIST', '/home/worker/local_dataset/FASHION_MNIST')
		en.mount_nfs (nfsApp, '/home/worker/dml_app')
	...
	emu4 = testbed.add_emulator ('emulator-4', '81.71.23.105', cpu=128, ram=256, unit='G')
	for j in range (4, 5):
		en = testbed.add_emulated_node ('n' + str (j), '/home/worker/dml_app',
			['python3', 'gl_peer.py'], 'dml:v1.0', cpu=1, ram=1, unit='G', emulator=emu4)
		# ${host_path} means /path/in/emulator-2/to/worker/dml_file in this example.
		en.mount_local_path ('./dml_file', '/home/worker/dml_file')
		en.mount_local_path ('./local_dataset/FASHION_MNIST', '/home/worker/local_dataset/FASHION_MNIST')
		en.mount_nfs (nfsApp, '/home/worker/dml_app')
	...
	# p1 = testbed.add_physical_node (name='p1', nic='eth0', ip='159.75.112.104')
	...
	# declare more physical node
	# which should run the worker/agent.py in advance.
	# p2 = testbed.add_physical_node (name='p2', nic='eth0', ip='81.71.23.105')
	# # ${mount_point} can use absolute path starting from / or
	# # relative path starting from the directory of the worker/agent.py file.
	# # ${mount_point} means /path/in/p1/to/worker/dml_app in this example.
	# p2.mount_nfs (nfs=nfsApp, mount_point='./dml_app')
	# p2.mount_nfs (nfsDataset, './dataset')
	# # set physical node's role.
	# # ${working_dir} can use absolute path or relative path as above mount_nfs ().
	# p2.set_cmd (working_dir='dml_app', cmd=['python3', 'gl_peer.py'])
