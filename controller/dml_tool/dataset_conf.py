...
if __name__ == '__main__':
	...
	parser.add_argument ('-d', '--dataset', dest='dataset', required=True, type=str,
		help='./relative/path/to/dataset/json/file') # 改 help='./relative/path/to/dataset/json/file'
	...
	for node_name in conf_json:
		...
		if 'test_len' not in node_conf: # trainer
			node_conf ['train_len'] = int(node_conf ['train_len']) # float->int
			node_conf ['test_len'] = -1
			node_conf ['test_start_index'] = -1

		if 'train_len' not in node_conf: # aggregator
			node_conf ['test_len'] = int(node_conf ['test_len']) # float->int
			node_conf ['train_len'] = -1
			node_conf ['train_start_index'] = -1
			node_conf ['data_size_of_trainers'] = {}
			for trainer_name in conf_json:
				trainer_conf = conf_json [trainer_name]
				if 'test_len' not in trainer_conf:
					node_conf ['data_size_of_trainers'].update({trainer_name: int(trainer_conf['train_len'])})
