...
class Conf:
	def __init__ (self, name):
		...
		self.sync = 0
		self.epoch = 0
		self.algorithm = "" # 模型算法，aggregator和trainer掌握
		self.parameters = {} # 算法超参数, aggregator和trainer掌握
		self.epoch_of_trainers = {} # 每个trainer的epoch，由aggregator掌握
		self.connect = {}
	...
	def to_json (self):
		if self.trainer_fraction != 0: # aggregator
			return {
				...
				'epoch_of_trainers': self.epoch_of_trainers 
			}
		else: # trainer
			return {
				'epoch': self.epoch,
...
def gen_conf (all_node, conf_json, link_json, output_path):
	...
	agg_conf.algorithm = conf_json ['algorithm']
	agg_conf.parameters = conf_json ['parameters']
	...
	for i in range (1, len (node_list)):
		...
		agg_conf.epoch_of_trainers.update({name:conf.epoch})
		conf.algorithm = conf_json ['algorithm']
		conf.parameters = conf_json ['parameters']
