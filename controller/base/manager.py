...
class Manager (metaclass=abc.ABCMeta):
	...
	def __load_default_route (self):
		...
		def route_conf_dataset ():
			...
			"""
			print("[debug]: route_conf_dataset()")
		...
		def route_start ():
			...
			"""
			# for name,emu in self.testbed.emulator.items():
			# 	emu_ip=emu.ipW
			# 	send_data("GET",'/cpulogstart',emu_ip,3333)
		...
		@self.testbed.flask.route ('/finish', methods=['GET', "POST"]) # 改
		def route_finish ():
			"""
	...
	def __send_conf (self, conf_type: str):
		...
		for pn in self.pNode.values ():
			...
			timer.start()
			# 1.压缩数据集（函数会判断是否需要压缩）：得到以nodeName_train/test.tar命名的压缩包
			if conf_type == 'dataset':
				compress_dataset('worker_' + pn.name, file_path) 
			# 2.发送配置文件
			with open (file_path, 'r') as f:
				print ('sent ' + conf_type + ' conf to ' + pn.name)
				send_data ('POST', '/conf/' + conf_type, pn.ip, pn.port, files={'conf': f})
			# 3.在worker会进行decompress和load操作
			costTime = round(timer.stop() / 60.0, 2)
			print('sent ' + conf_type + ' conf to ' + pn.name + ' cost ' + str(costTime) + ' mins.')
		...
		for en in self.eNode.values ():
			...
			# 1.压缩数据集（函数会判断是否需要压缩）
			if conf_type == 'dataset':
				compress_dataset('worker_' + en.name, file_path)		
			# 2.发送配置文件
			with open (file_path, 'r') as f:
			...
			# 3.在worker会进行decompress和load操作
			costTime = round(timer.stop() / 60.0, 2)
			print('sent ' + conf_type + ' conf to ' + en.name + ' cost ' + str(costTime) + ' mins.')

		totTime = round(timer.sum() / 60.0, 2)
		print('sent ' + conf_type + ' conf to nodes costs ' + str(totTime) + ' mins.')
